...
Error ReSTIRDI::init()
{
	...
	m_phase1ReservoirsRtDesc = getRenderer().create2DRenderTargetDescription(
		getRenderer().getInternalResolution().x, getRenderer().getInternalResolution().y, Format::kR32G32B32A32_Uint, "ReSTIRDI: Phase 1 Reservoirs");
	m_phase1ReservoirsRtDesc.bake();
...
void ReSTIRDI::populateRenderGraph()
{
	...
	const RenderTargetHandle phase1ReservoirsHandle = rgraph.newRenderTarget(m_phase1ReservoirsRtDesc);

	// Phase 1 reads the history and phase 2 writes the one that becomes next frame's history
	const U32 readIdx = getRenderer().getFrameCount() & 1;
	const RenderTargetHandle historyReservoirHandle =
		rgraph.importRenderTarget(m_reservoirTextures[readIdx].get(), m_texturesFirstImport, TextureUsageBit::kSrvCompute);
	const RenderTargetHandle finalReservoirHandle =
		rgraph.importRenderTarget(m_reservoirTextures[!readIdx].get(), m_texturesFirstImport, TextureUsageBit::kSrvCompute);
	m_texturesFirstImport = false;

	// Phase 1: initial candidates, their visibility and temporal re-use
	{
		NonGraphicsRenderPass& rpass = rgraph.newNonGraphicsRenderPass("ReSTIRDI: Phase 1");

		rpass.newTextureDependency(phase1ReservoirsHandle, TextureUsageBit::kUavCompute);
		rpass.newTextureDependency(historyReservoirHandle, TextureUsageBit::kSrvCompute);
		...
		rpass.setWork([this, phase1ReservoirsHandle, historyReservoirHandle](RenderPassWorkContext& rgraphCtx) {
			ANKI_TRACE_SCOPED_EVENT(ReSTIRDI);
			CommandBuffer& cmdb = *rgraphCtx.m_commandBuffer;

			cmdb.bindShaderProgram(m_phase1GrProg.get());
	...
	// Phase 2: spatial re-use and shading
	{
		NonGraphicsRenderPass& rpass = rgraph.newNonGraphicsRenderPass("ReSTIRDI: Phase 2");

		rpass.newTextureDependency(finalReservoirHandle, TextureUsageBit::kUavCompute);
		rpass.newTextureDependency(m_runCtx.m_rt, TextureUsageBit::kUavCompute);
		rpass.newTextureDependency(phase1ReservoirsHandle, TextureUsageBit::kSrvCompute);
		...
		rpass.setWork([this, phase1ReservoirsHandle, finalReservoirHandle](RenderPassWorkContext& rgraphCtx) {
			ANKI_TRACE_SCOPED_EVENT(ReSTIRDI);
			CommandBuffer& cmdb = *rgraphCtx.m_commandBuffer;

			cmdb.bindShaderProgram(m_phase2GrProg.get());
			...
			const UVec4 consts(g_cvarRenderReSTIRDISpatialReuseSampleCount, asU32(F32(g_cvarRenderReSTIRDISpatialReuseSampleRadius)), 0u, 0u);
