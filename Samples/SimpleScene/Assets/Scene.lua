-- Generated by: C:\src\anki\build_dbg_vk\Binaries\GltfImporter.exe CornellBox.gltf .. -rpath Assets -texrpath Assets -v -j 0 -light-scale 0.01839878467931728
local scene = getSceneGraph()
local events = getEventManager()

node = scene:newSceneNode("Mesh_0")
node:newMeshComponent():setMeshFilename("Assets/Mesh_0_d56f58fc33de003f.ankimesh")
node:newMaterialComponent():setMaterialFilename("Assets/backWall_361f28d87a6738d3.ankimtl"):setSubmeshIndex(0)
trf = Transform.new()
trf:setOrigin(Vec3.new(0.000000, 0.000000, 0.000000))
rot = Mat3.new()
rot:setAll(1.000000, 0.000000, 0.000000, 0.000000, -0.000000, 1.000000, 0.000000, -1.000000, -0.000000)
trf:setRotation(rot)
trf:setScale(Vec3.new(5.000000, 5.000000, 5.000000))
node:setLocalTransform(trf)

node = scene:newSceneNode("Mesh_1")
node:newMeshComponent():setMeshFilename("Assets/Mesh_1_266a0dd9d2092f46.ankimesh")
node:newMaterialComponent():setMaterialFilename("Assets/ceiling_3fd94cde277a48e1.ankimtl"):setSubmeshIndex(0)
trf = Transform.new()
trf:setOrigin(Vec3.new(0.000000, 0.000000, 0.000000))
rot = Mat3.new()
rot:setAll(1.000000, 0.000000, 0.000000, 0.000000, -0.000000, 1.000000, 0.000000, -1.000000, -0.000000)
trf:setRotation(rot)
trf:setScale(Vec3.new(5.000000, 5.000000, 5.000000))
node:setLocalTransform(trf)

node = scene:newSceneNode("Mesh_2")
node:newMeshComponent():setMeshFilename("Assets/Mesh_2_be53007bec464649.ankimesh")
node:newMaterialComponent():setMaterialFilename("Assets/floor_71cbd2644e53ab8c.ankimtl"):setSubmeshIndex(0)
trf = Transform.new()
trf:setOrigin(Vec3.new(0.000000, 0.000000, 0.000000))
rot = Mat3.new()
rot:setAll(1.000000, 0.000000, 0.000000, 0.000000, -0.000000, 1.000000, 0.000000, -1.000000, -0.000000)
trf:setRotation(rot)
trf:setScale(Vec3.new(5.000000, 5.000000, 5.000000))
node:setLocalTransform(trf)

node = scene:newSceneNode("Mesh_3")
node:newMeshComponent():setMeshFilename("Assets/Mesh_3_c026fdb5b74773ed.ankimesh")
node:newMaterialComponent():setMaterialFilename("Assets/leftWall_fe97b196ed148fca.ankimtl"):setSubmeshIndex(0)
trf = Transform.new()
trf:setOrigin(Vec3.new(0.000000, 0.000000, 0.000000))
rot = Mat3.new()
rot:setAll(1.000000, 0.000000, 0.000000, 0.000000, -0.000000, 1.000000, 0.000000, -1.000000, -0.000000)
trf:setRotation(rot)
trf:setScale(Vec3.new(5.000000, 5.000000, 5.000000))
node:setLocalTransform(trf)

node = scene:newSceneNode("Mesh_4")
node:newMeshComponent():setMeshFilename("Assets/Mesh_4_4d4aae6c030c4fd5.ankimesh")
node:newMaterialComponent():setMaterialFilename("Assets/light_1544a10dffc35038.ankimtl"):setSubmeshIndex(0)
trf = Transform.new()
trf:setOrigin(Vec3.new(0.000000, 0.000000, 0.000000))
rot = Mat3.new()
rot:setAll(1.000000, 0.000000, 0.000000, 0.000000, -0.000000, 1.000000, 0.000000, -1.000000, -0.000000)
trf:setRotation(rot)
trf:setScale(Vec3.new(5.000000, 5.000000, 5.000000))
node:setLocalTransform(trf)

node = scene:newSceneNode("Mesh_5")
node:newMeshComponent():setMeshFilename("Assets/Mesh_5_629309b27fa549a7.ankimesh")
node:newMaterialComponent():setMaterialFilename("Assets/rightWall_d627c19b8205864.ankimtl"):setSubmeshIndex(0)
trf = Transform.new()
trf:setOrigin(Vec3.new(0.000000, 0.000000, 0.000000))
rot = Mat3.new()
rot:setAll(1.000000, 0.000000, 0.000000, 0.000000, -0.000000, 1.000000, 0.000000, -1.000000, -0.000000)
trf:setRotation(rot)
trf:setScale(Vec3.new(5.000000, 5.000000, 5.000000))
node:setLocalTransform(trf)

node = scene:newSceneNode("Mesh_7")
node:newMeshComponent():setMeshFilename("Assets/Mesh_7.001_95b61c94f8a0ad1c.ankimesh")
node:newMaterialComponent():setMaterialFilename("Assets/tallBox_15316a0c94bdf7f6.ankimtl"):setSubmeshIndex(0)
trf = Transform.new()
trf:setOrigin(Vec3.new(-1.675000, 3.600000, -1.450001))
rot = Mat3.new()
rot:setAll(0.958636, 0.000000, 0.284634, 0.000000, 1.000000, 0.000000, -0.284634, 0.000000, 0.958636)
trf:setRotation(rot)
trf:setScale(Vec3.new(5.000000, 5.000000, 5.000000))
node:setLocalTransform(trf)

node = scene:newSceneNode("Mesh_6")
node:newMeshComponent():setMeshFilename("Assets/Mesh_6.001_2550937d23ca3066.ankimesh")
node:newMaterialComponent():setMaterialFilename("Assets/shortBox_122467965d493dab.ankimtl"):setSubmeshIndex(0)
trf = Transform.new()
trf:setOrigin(Vec3.new(1.637500, 1.800000, 1.862499))
rot = Mat3.new()
rot:setAll(0.966822, 0.000000, -0.255450, 0.000000, 1.000000, 0.000000, 0.255450, 0.000000, 0.966822)
trf:setRotation(rot)
trf:setScale(Vec3.new(5.000000, 5.000000, 5.000000))
node:setLocalTransform(trf)

node = scene:newSceneNode("Camera")
scene:setActiveCameraNode(node)
comp = node:newCameraComponent()
comp:setPerspective(0.100000, 500.000000, getRenderer():getAspectRatio() * 1.024779, 1.024779)
trf = Transform.new()
trf:setOrigin(Vec3.new(0.217066, 6.668793, 17.325689))
rot = Mat3.new()
rot:setAll(0.999854, -0.000334, 0.017085, -0.001690, 0.992972, 0.118341, -0.017005, -0.118352, 0.992826)
trf:setRotation(rot)
trf:setScale(Vec3.new(1.000000, 1.000000, 1.000000))
node:setLocalTransform(trf)

node = scene:newSceneNode("Point")
lcomp = node:newLightComponent()
lcomp:setLightComponentType(LightComponentType.kPoint)
lcomp:setColor(Vec3.new(1.000000, 1.000000, 1.000000))
lcomp:setLuminousPower(125.663706)
lcomp:setShadowEnabled(true)
lcomp:setInfluenceRadius(20.000000)
trf = Transform.new()
trf:setOrigin(Vec3.new(0.000000, 8.913514, 0.000000))
rot = Mat3.new()
rot:setAll(1.000000, 0.000000, 0.000000, 0.000000, -0.000000, 1.000000, 0.000000, -1.000000, -0.000000)
trf:setRotation(rot)
trf:setScale(Vec3.new(1.000000, 1.000000, 1.000000))
node:setLocalTransform(trf)

node = scene:newSceneNode("Cube.011")
comp = node:newGlobalIlluminationProbeComponent()
trf = Transform.new()
trf:setOrigin(Vec3.new(-0.064907, 4.847455, -0.144611))
rot = Mat3.new()
rot:setAll(1.000000, 0.000000, 0.000000, 0.000000, 1.000000, 0.000000, 0.000000, 0.000000, 1.000000)
trf:setRotation(rot)
trf:setScale(Vec3.new(5.912430, 5.785918, 5.520432))
node:setLocalTransform(trf)
